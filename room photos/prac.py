...
def mainfunc():
    ...
    cur=con.cursor()
    cur.execute("create table if not exists Emp(name varchar(20),sex varchar(20),age number,nationality varchar(20),purpose varchar(20),mobile number,identity number,roomtype varchar(20),roomno number)")    
    root=Tk()
    ...
    Label(root,text="Enter your sex",font='arial 15 bold').grid(row=20,column=0,sticky=W)
    ...
    Label(root,text="Enter your identification",font="arial 20 bold").grid(row=10,column=10)
    x=IntVar()
    Radiobutton(root,text='driving licence no.',variable=x,value=1,font='arial 15 bold').grid(row=15,column=10,sticky=W)
    Radiobutton(root,text='aadhar card no.',variable=x,value=2,font='arial 15 bold').grid(row=20,column=10,sticky=W)
    Radiobutton(root,text='Pan card no.',variable=x,value=3,font='arial 15 bold').grid(row=25,column=10,sticky=W)
    Radiobutton(root,text='Voter ID card no.',variable=x,value=4,font='arial 15 bold').grid(row=30,column=10,sticky=W)
    Label(root,text="Enter one ID number :",font='arial 15 bold').grid(row=35,column=10,sticky=W)
    l=Entry(root,width=25,bd=5,bg='orange')
    l.grid(row=35,column=11)
    ...
    Label(root,text="select room number",font="arial 20 bold italic").grid(row=60,column=0)
    var=IntVar()
    var.set("room no.")
    x=OptionMenu(root,var,"201","202","203","204","205").grid(row=62,column=0)
    x=Entry(root,width=25,bd=5,bg='orange')
    x.grid(row=62,column=5)    
    def insert():
        a=(e.get(),d.get(),int(f.get()),g.get(),h.get(),int(i.get()),int(l.get()),n.get(),int(x.get()))
    ...
    def showall():
        ...
        print a
    def viewroom1():
        os.startfile("scrollbar_canvas.py")
    ...
    Button(root,text="View Rooms",command=viewroom1,bd=5).grid(row=50,column=0)
    root.mainloop()
