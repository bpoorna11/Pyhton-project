...
def mainfunc():
    ...
    cur=con.cursor()
    cur.execute("create table if not exists Emp(name varchar(20) primary key,sex varchar(20),age number,nationality varchar(20),purpose varchar(20),mobile number,identity varchar(20),roomtype varchar(20),roomno number)")    
    root=Tk()
    ...
    def delete():
        ...
        cur.execute("delete from Emp where name='yashi'")
        con.commit()
        showall()
    def update():
        cur.execute("update Emp set roomtype='double room' where name='yashi'")
        con.commit()
        showall()
    def find():
        cur.execute("select * from Emp where name='Richa'")
        showall()
    def viewroom1():
        root2=Toplevel()
    ...
    def viewroom4():
        ...
        root5.mainloop()
    Button(root,text='Book',bd=5,height=2,width=8,command=insert,relief=RAISED).grid(row=65,column=2)
    Button(root,text='Show',bd=5,height=2,width=8,command=show,relief=RAISED).grid(row=65,column=3)
    Button(root,text='Show All',bd=5,height=2,width=8,command=showall,relief=RAISED).grid(row=65,column=4)
    Button(root,text='Edit',bd=5,height=2,width=8,command=update,relief=RAISED).grid(row=65,column=5)
    Button(root,text='delete',bd=5,height=2,width=8,command=delete,relief=RAISED).grid(row=65,column=6)
    Button(root,text='Find',bd=5,height=2,width=8,command=find,relief=RAISED).grid(row=65,column=7)
